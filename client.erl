...
do_join(State, Ref, ChatName) ->
	case maps:is_key(ChatName, State#cl_st.con_ch) of
		...
		false ->
			...
			receive
				{ChatRoomPID, _Ref, connect, ChatHistory} ->
					Rooms = maps:put(ChatName, ChatRoomPID, State#cl_st.con_ch),
					{ChatHistory, State#cl_st{con_ch = Rooms}}
			end
	end.
		...
		true ->
			...
			receive
				{_From, _Ref, ack_leave} ->
					Rooms = maps:remove(ChatName, State#cl_st.con_ch),
					{ok, State#cl_st{con_ch = Rooms}}
			end;
		false ->
			{err, State}
end.
...
do_msg_send(State, Ref, ChatName, Message) ->
	maps:get(ChatName, State#cl_st.con_ch) ! {self(), Ref, message, Message},
